<template>
  <div class="layout-root">
    <aside class="sidebar desktop-only">
      ...
      <nav class="nav-list">
        <Button
          ...
          :label="item.label"
          :icon="item.icon"
          :severity="activeView === item.key ? 'contrast' : undefined"
          text
          class="nav-btn"
    ...
    <Sidebar v-model:visible="mobileSidebarOpen" position="left" class="mobile-sidebar">
      <template #header>
    ...
    <div class="content-area">
      <header class="topbar">
        <Button icon="pi pi-bars" text class="mobile-menu-btn" @click="mobileSidebarOpen = true" />
        <h2>{{ viewTitle }}</h2>
      </header>
